...
class IssueFaculty extends JPanel implements ActionListener
{
	
	JButton issue,clear,close;
	JLabel l_department,l_name,l_bookSNo,l_bookName,l_author,l_IssueDate,l_sub;
	JComboBox dept;
	JTextField t_name,t_bookSNo,t_bookName,t_author,t_IssueDate,t_sub;
	...
	public IssueFaculty(Connection c)
	{
		con = c;
		...
		JLabel img = new JLabel(new ImageIcon("Images/issueFaculty.jpg"));
		imgPanel.add(img);
	...
	public void actionIssue()
	{
		...
		else
		{
			try
			{
				String str = "SELECT * FROM issue_faculty WHERE department=? AND name=? AND submit_date is null";
				...
				else
				{
					...
					String s = "INSERT INTO issue_faculty (department,name,bookno,issue_date) values(?,?,?,?)";
